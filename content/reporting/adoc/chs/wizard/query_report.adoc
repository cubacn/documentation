...
执行完上述操作后，添加可直接从图书馆图书类目浏览界面运行报表的 *Run report* 按钮。

为此，需要在 `librarydepartment-browse.xml` 界面描述中定义一个新的 <<run_report_action,RunReportAction>> {main_man_url}/standard_actions.html[标准操作] 以及一个按钮：
