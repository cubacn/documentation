...
从工具箱将 `Table` 组件拖至组件树面板，放置在 `label` 和 `editActions` 中间。在组件结构（Hierarchy）面板选中此组件，然后在 *Properties* 标签页设定表格的大小：在 *width* 字段设置 `300px`，在 *height* 字段设置 `200px`。从可用的数据容器中选择 `ordersDc`。然后使用 *id* 字段边上的 image:generate_id.png[] 按钮来生成表格的标识符：`ordersTable`。
