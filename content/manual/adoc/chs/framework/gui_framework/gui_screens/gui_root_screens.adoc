...
* <<cuba.web.appWindowMode,cuba.web.appWindowMode>> – 设置主窗口的默认模式：标签页式的还是单独界面式（`TABBED` 或 `SINGLE`）。用户可以使用 `UserActionsButton` 提供的 *Settings - 设置* 界面进行修改。

* <<cuba.web.maxTabCount,cuba.web.maxTabCount>> – 当主界面在标签页式时，使用该属性设置用户能打开的标签页最多个数。默认值为 7。

* <<cuba.web.foldersPaneEnabled,cuba.web.foldersPaneEnabled>> - 为使用 *Main screen with top menu* 模板创建的界面启用显示文件夹面板。
