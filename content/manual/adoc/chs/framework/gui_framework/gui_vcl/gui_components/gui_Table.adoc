...
++++
<div class="manual-live-demo-container">
    <a href="http://files.cuba-platform.com/javadoc/cuba/7.1/com/haulmont/cuba/gui/components/Table.html" class="api-docs-btn" target="_blank">API 文档</a>
</div>
++++
...
* 如果 `columnControlVisible` 属性设置为 `false`，则用户无法使用位于表头的右侧的下拉菜单按钮 image:gui_table_columnControl.png[] 隐藏列：image:gui_table_columnControl.png[] 按钮位于表头的右侧。当前显示的列在菜单中标记为选中状态。
...
////
为此，`getItemDatasource()` 方法用于从表中获取当前实体实例的数据源，并将其传递给 `LookupPickerField` 组件。
////
