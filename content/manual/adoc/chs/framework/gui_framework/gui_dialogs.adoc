...
. 带有 `Optional` 类型结果值的输入对话框：
+
[source, java]
----
include::{sourcesdir}/gui/dialogs/gui_input_dialog_6.java[]
----
<1> - `getOptional()` 方法返回一个 `Optional` 结果值，可以用参数标识符获取。
+
用户可以从 `InputDialog`、`InputDialogCloseEvent`、`InputDialogResult` 和 `ValidationContext` 获取 `Optional` 类型结果值。
--
