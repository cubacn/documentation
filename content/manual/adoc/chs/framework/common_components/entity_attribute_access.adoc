...
* 当 <<dataManager,DataManager>> 加载一个实体时，它会找到实现 `SetupAttributeAccessHandler` 接口的所有托管 bean，并传递 `SetupAttributeAccessEvent` 对象作为参数调用它们的 `setupAccess()` 方法。此对象包含处于托管状态的已加载实例，以及三个用于存储属性名称的集合：只读属性集合、隐藏属性集合和必须属性集合（这些集合最初为空）。
...
* 在项目的 *core* 模块中创建<<managed_beans,托管 Bean>> 并实现 `SetupAttributeAccessHandler` 接口。使用被处理实体的类型对接口进行参数化。bean 范围(scope)必须是默认的单例(singleton)。必须实现接口方法：
