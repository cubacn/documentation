...
+
--
如果设置为 `true`，框架会在应用程序启动时检查所有额外数据存储的连接。如果连接失败，日志会记录失败消息。注意，这种检查会使得启动过程变慢。
...
+
--
如果设置为 `true`，在 Middleware 开启实体属性权限检查。如果设置为 `false`，属性权限检查则在客户端层做，比如在 <<gui_framework,Generic UI>> 或者 {rest_man_url}[REST API]。
